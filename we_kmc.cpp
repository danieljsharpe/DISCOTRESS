...
WE_KMC::WE_KMC(const Network &ktn, int n_abpaths, int maxit, double tau, int nbins, \
...
void WE_KMC::run_enhanced_kmc(const Network &ktn) {
    ...
    int n_ab=0, n_wekmcit=0;
    double tau_r=tau; // next resampling time
    // setup walkers (set nwalkers here)
    cout << "wekmc> finished initialising walkers" << endl;
    while ((n_ab<n_abpaths) and (n_wekmcit<maxit)) { // algorithm terminates when max. no. of iterations of resampling procedure have been performed
